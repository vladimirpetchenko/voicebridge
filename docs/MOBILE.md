...
- 🔮 **Голосовой ввод (план).** Запись с микрофона на телефоне → отправка аудио
  на десктоп → распознавание (whisper уже есть на десктопе) → промпт в сессию.
  Неочевидно, сработает ли: поток аудио поверх WebSocket, права микрофона на
  iOS/Android.
...
- `get_git_changes` — сводка Git проекта выбранной сессии: объект `GitInfo`
  (`branch` — текущая ветка, `changes` — массив `GitFileChange` со полями
  `path/status/additions/deletions`); `sessionId` необязателен (по умолчанию —
  выбранная сессия).
...
{ "type": "event", "name": "git-changes", "data": { "sessionId": "…", "branch": "main", "changes": [ { "path": "…", "status": "modified", "additions": 1, "deletions": 1 } ] } }
```
...
5. ⏳ **Этап 4 — вне LAN**: Tailscale/ZeroTier (или облачный relay) для доступа
   из любой сети, пуши.
6. 🔮 **Этап 5 — голосовой ввод**: запись на телефоне → аудио на десктоп →
   распознавание (whisper) → промпт. Неочевидно, сработает ли (поток аудио по
   WS, права микрофона iOS/Android).
