...
- **Окно чата** (по одному на сессию) — основной интерфейс общения: сверху
  ...
  (разрешения/вопросы), история сессии. У каждого ответа — кнопка копирования;
  пока OpenCode думает, показывается анимированный индикатор. Внизу — панель
  ввода: текстовое поле + кнопка отправки + голосовая кнопка (как в Telegram).
  Ниже — строка состояния: токены сессии и стоимость.
  Окна изолированы друг от друга по сессиям.
