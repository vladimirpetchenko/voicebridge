...
4. **Панель сообщений (десктоп)** — список сообщений диалога с переходом к
   любому сообщению (навигация по чату).
5. **Скролл чата** — не автоскроллить вниз, пока ответ OpenCode продолжает
   стримиться: позиция «замирает» (можно читать историю); к низу — по явному
   действию/кнопке.
6. **Управление сессиями** — удаление сессии из лаунчера (десктоп + мобилка) и
   автоматическое название: если сессия была пустой (без заголовка), после
   первого сообщения подставлять осмысленное имя.
7. **GUI-автоматизация** — вставка распознанного текста в выбранное окно.
