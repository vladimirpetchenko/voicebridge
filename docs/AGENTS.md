...
мобилке — tooltip по долгому нажатию. Внизу окна чата — строка состояния
(модель, токены сессии, стоимость) из `GET /session/{id}`. В футере лаунчера —
название выбранной сессии, а когда сессия не выбрана — версия приложения.
Деструктивные действия (скрытие проекта, удаление сессии, обновление)
подтверждаются через общий `ConfirmDialog` (десктоп,
`src/shared/ui/ConfirmDialog.tsx`) / `showConfirmDialog` (мобилка). Горячая клавиша
...
- ✅ **Восстановление ответа в мобилке при возврате** — стриминговый ответ
  больше не теряется при сворачивании: при возврате из фона (и после
  переподключения) диалог подтягивается из истории десктопа/OpenCode
  (`get_conversation`), индикатор «думает» снимается, если генерация
  завершилась. `AppController` подписан на жизненный цикл
  (`WidgetsBindingObserver`, `didChangeAppLifecycleState`), `loadConversation`
  возвращает признак успеха.
...
2. **Баг: ошибка OpenCode рвёт сессию** — когда OpenCode присылает ошибку, в
   лаунчере появляется красное сообщение (ошибка в сокетах OpenCode), сессия
   чата прерывается, хотя текст ответа продолжает дописываться. Нужно не
   показывать ложную «ошибку»/не рвать стрим, если ответ реально продолжает
   приходить (`opencode-error` vs продолжающийся `opencode-delta`).
3. **Панель скриптов проекта** — показывать в проекте доступные для запуска
   скрипты (`*.sh`, на Windows — `*.bat`/`*.cmd`/`*.ps1`) и запускать их по
   клику с подтверждением.
